...
const [deleteLoading,setDeleteLoading]=useState(null)
  ...
  const GETDATA = async () => {
    try {
      ...
      if (result.status) {
        dispatch(setParentspreview(result)) 
                   ...
                   <div className="">
                     ...
                     <div className="space-y-4">
                       <select
                         ...
                         className="w-full border-[1px] p-1 capitalize"
                       >
                         <option value="">My Favorite Food</option>
                         <option value="other">Custom</option>
...
{errors[index]?.birth_city && <p className="text-red-500">{errors[index]?.birth_city}</p>}
                   ...
                   <div className={`grid ${otherChild[index]?.favourite_food ==="other" ? "grid-cols-2" :"grid-cols-1"} gap-x-3`}>
                     <div>
                       A Favorite Food For {otherChild[index]?.full_name}  
                       <div className="space-y-4">
                         <select
                           name="favourite_food"
                           id=""
                           ...
                           className="w-full border-[1px] p-1"
                         >
                           <option value="">My Favorite Food For </option>{" "}
                           <option  value="other">Custom </option>
                           {/* Placeholder */}
                           {textFood.map((buttonText, index) => (
                             <option key={index} value={buttonText}>
               ...
               parents   && ( */}
                     <button
                       type="button"
                       className="h-[40px] w-[30%] md:w-[27%] montserrat bg-[#0069D9] text-base leading-4 font-normal hover:bg-gray-400 text-white py-3 px-4 rounded inline-flex items-center"
